...
if(isset($_POST["tambah"])){
  ...
  $tambah = $db->insert('user',['id_user'=>$id,'email'=>$email,'password'=>$password,'user_fullname'=>$fullname,'no_telp'=>$telfon,'gender'=>$gender,'alamat'=>$alamat]);
...
if ($tambah){
  $upload = $img->upFoto('user',$imgNama,$size,$asal,$format,"id_user='$id'");
    ...
    ?>
      <script>
        alert("Selamat data user <?=$fullname?> berhasil di Tambah");
    ...
    ?>
      <script>
        alert("Maaf Upload foto <?=$fullname?> gagal dimasukkan");
      </script>
    <?php
  ...
  ?>
  <script>
    alert("Maaf data user <?=$fullname?> Gagal di tambah");
